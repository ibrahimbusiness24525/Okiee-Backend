  ...
  try {
    await session.withTransaction(async () => {
      ...
      if (!estimatedAmount || estimatedAmount <= 0) {
        throw new Error("Estimated amount is required and must be greater than 0");
      }
      ...
      if (!modelDoc) {
        throw new Error("Model not found or does not belong to the specified company");
      }
      ...
      if (paymentType === "credit") {
        ...
        if (!person) {
          ...
          const balanceAmount = Math.abs(updatedGivingCredit - updatedTakingCredit);
